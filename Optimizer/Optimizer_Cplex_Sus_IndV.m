classdef Optimizer_Cplex_Sus_IndV < handle
    properties
        T;
        Code;
        Alpha;
    ...
    methods
        function obj=Optimizer_Cplex_Sus_IndV(Input)
            obj.T=Input.Time;
            ...
            obj.IndMin=IndExp'*(IX_NonTrade.*AllPre-AllBench);
            if max(obj.IndMin)>obj.Cons.Inds(2)
                disp('Ind exposure cannot be satisfied')
            end
            obj.Code=AllCode;
            obj.Alpha=AllAlpha;
            obj.PreHold=AllPre;
