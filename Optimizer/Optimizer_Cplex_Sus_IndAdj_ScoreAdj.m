classdef Optimizer_Cplex_Sus_IndAdj_ScoreAdj < handle
    properties
        T;
        Code;
        Alpha;
    ...
    methods
        function obj=Optimizer_Cplex_Sus_IndAdj_ScoreAdj(Input)
            obj.T=Input.Time;
        ...
        function Optimize(obj)
            ...
            %%
            [~, ExpCorrC] = cov2corr(Q);
            D=(0.5*(1-ExpCorrC)).^0.5;
            Z = linkage(D,'single');
            
            %
            L=length(D(:,1));
            OrderList=Z(end,1:2);
            while max(OrderList)>L
                K=find(OrderList==max(OrderList));
                V=OrderList(K);
                VRK=V-L;
                VN=Z(VRK,1:2);
                OrderList=[OrderList(1:K),0,OrderList(K+1:end)];
                OrderList(K:K+1)=VN;
            end
            Result=signalOrderAdj(obj.Alpha,OrderList);
            r=Result'*obj.AveR;
            %%
