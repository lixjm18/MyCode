classdef Optimizer_Cplex_Sus_Ind_Norm < handle
    properties
        T;
        Code;
        Alpha;
    ...
    methods
        function obj=Optimizer_Cplex_Sus_Ind_Norm(Input)
            obj.T=Input.Time;
            ...
            AllAlpha=nan(size(AllCode));
            [~,IA,IB]=intersect(AlphaCode,AllCode,'stable');
            AllAlpha(IB)=AlphaV(IA);
            IndCode0=Input.Risks.StockRiskRaw.InnerCode;
            Ind0=Input.Risks.StockRiskRaw.FirstIndustryCode;
            AllInd=nan(size(AllCode));
            [~,IA,IB]=intersect(IndCode0,AllCode,'stable');
            AllInd(IB)=Ind0(IA);
            AllInd(isnan(AllInd))=0;
            AllAlpha=get_rank_score_ind_neu(AllAlpha,AllInd,0);
