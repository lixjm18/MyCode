classdef Optimizer_Cplex_Sus_IndAdj_InnerCons < handle
    properties
        T;
        Code;
        Alpha;
    ...
    methods
        function obj=Optimizer_Cplex_Sus_IndAdj_InnerCons(Input)
            obj.T=Input.Time;
...
%                     eval(['C=C+[',num2str(lb),'<=obj.StyleExp.',Name,'''*W<=',num2str(rb),'];']);
                     eval(['C=C+[',num2str(lb),'*sum(obj.BenchW.*obj.IndExp(:,',num2str(i0),'))<=obj.StyleExp.',Name,'''.*obj.IndExp(:,',num2str(i0),')''*W<=',num2str(rb),'*sum(obj.BenchW.*obj.IndExp(:,',num2str(i0),'))];']);
                end
            end
